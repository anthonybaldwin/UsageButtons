...
| OpenAI Admin | OpenAI admin API key (`sk-admin-…`) from the Provider tab or `OPENAI_ADMIN_API_KEY`. Org admins only — personal `sk-` keys won't work. | Org spend today (UTC, $), org spend month-to-date ($), org spend last 30 days ($). Distinct from the Codex provider, which is a single user's session/weekly window from local OAuth. |
