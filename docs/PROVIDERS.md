...
| Anthropic | Anthropic admin API key (`sk-ant-admin-…`) from the Provider tab or `ANTHROPIC_ADMIN_API_KEY` (kept namespaced so it doesn't shadow the SDK-standard `ANTHROPIC_API_KEY`). Org admins only — personal `sk-ant-` keys are rejected by the admin endpoints. | Org spend today (UTC, $), org spend month-to-date ($), org spend last 30 days ($). Distinct from the Clawd provider (per-user session/weekly window from local OAuth) — this is the org-wide cost view. |
...
| DeepSeek | DeepSeek API key from the Provider tab or `DEEPSEEK_API_KEY` / `DEEPSEEK_KEY`. | Total balance ($, USD preferred), paid balance ($), granted balance ($). |
...
| OpenAI | OpenAI admin API key (`sk-admin-…`) from the Provider tab or `OPENAI_ADMIN_API_KEY` (kept namespaced so it doesn't shadow the SDK-standard `OPENAI_API_KEY`). Org admins only — personal `sk-` keys are rejected by the admin endpoints. | Org spend today (UTC, $), org spend month-to-date ($), org spend last 30 days ($). Distinct from the Codex provider (per-user session/weekly window from ChatGPT OAuth) — this is the org-wide cost view. |
