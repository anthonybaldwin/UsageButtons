...
func (Provider) Fetch(_ providers.FetchContext) (providers.Snapshot, error) {
	...
	if token == "" {
		return providers.Snapshot{
			...
			Status:       "unknown",
			Error:        "Enter a GitHub token in the Copilot tab, set GITHUB_TOKEN, or provide ~/.config/github-copilot/hosts.json or apps.json.",
		}, nil
	}
	...
	if err != nil {
		...
		if errors.As(err, &httpErr) && (httpErr.Status == 401 || httpErr.Status == 403) {
			return providers.Snapshot{
				...
				Metrics:      []providers.MetricValue{},
				Status:       "unknown",
				Error:        "GitHub token unauthorized for Copilot. Update the token in the Copilot tab, GITHUB_TOKEN, or the github-copilot config files.",
			}, nil
		}
