...
//
// FetchContext.ActiveMetricIDs is intentionally ignored: both emitted
// metrics — session-percent (request quota) and weekly-percent (token
// quota) — are derived from the same usage-vs-limit Cloud Monitoring
// time-series pair, so filtering by the active set wouldn't reduce
// traffic. If Vertex ever grows a per-metric endpoint (e.g. a separate
// TPU-quota series), revisit and honour the active set here.
